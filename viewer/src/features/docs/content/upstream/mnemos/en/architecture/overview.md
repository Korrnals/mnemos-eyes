---
title: Mnemos — System Architecture
...
> Curator: how Mnemos is built — hybrid memory, the CLI/HTTP/MCP surfaces, and where vesmaro-eyes fits.
...
and an Obsidian-compatible vault. The Web UI for Mnemos is **vesmaro-eyes** — the board that serves this documentation.
...
When `cache_aligner.enabled` is `false`, `align_prefix()` returns the text unchanged with an empty `extracted` list. The MCP tool `mnemos_align_prefix` is the public surface; see [mcp-tools.md#mnemos_align_prefix](mnemos/user/mcp-tools#mnemos_align_prefix).
...
See [mcp-tools.md#output-token-reduction-p1-7](mnemos/user/mcp-tools#output-token-reduction-p1-7) for the user-facing reference.
...
#### MCP Server
Tools for Copilot / LLM agents (full catalogue in [mcp-tools.md](mnemos/user/mcp-tools)):
