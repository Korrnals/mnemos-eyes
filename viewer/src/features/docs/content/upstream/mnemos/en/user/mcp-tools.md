...
> Curator: all mnemos_* tools for MCP clients — the same layer agents on the board use to work with memory.
...
For a quick start on wiring it into VS Code, see [getting-started.md#run-the-mcp-server](mnemos/user/getting-started#connect-your-harness-mcp). For programmatic access, the same capabilities are also available over HTTP — see [http-api.md](mnemos/user/http-api). For the tag schema enforced by most tools, see [tag-contract.md](mnemos/user/tag-contract).
...
Create a new memory entry. The MCP layer enforces the Mnemos tag contract ([M2](mnemos/user/tag-contract)) before writing.
...
| `❌ Error: ...` | SQLite write failure, vault write failure, or embed failure (the latter is non-fatal — see [architecture overview](mnemos/architecture/overview#1-storage-layer)). |

### Related

- Tag schema: [tag-contract.md](mnemos/user/tag-contract)
- HTTP equivalent: [`POST /memories`](mnemos/user/http-api#post-memories--create-memory)
- CLI equivalent: [`mnemos add`](mnemos/user/cli-reference#add)
...
- HTTP equivalent: [`POST /search`](mnemos/user/http-api#search)
- CLI equivalent: [`mnemos search`](mnemos/user/cli-reference#search)
...
- HTTP equivalent: [`GET /recall/agent/{name}`](mnemos/user/http-api#get-recallagentname--agent-recall)
- CLI equivalent: [`mnemos recall --agent <slug>`](mnemos/user/cli-reference#recall)
...
- HTTP equivalent: [`POST /context/recall`](mnemos/user/http-api#post-contextrecall--recall-session-context)
...
- HTTP equivalent: [`POST /context/save`](mnemos/user/http-api#post-contextsave--save-a-session-checkpoint)
...
- HTTP equivalent: [`GET /memories`](mnemos/user/http-api#get-memories--list-recent)
- CLI equivalent: [`mnemos recall`](mnemos/user/cli-reference#recall)
...
- HTTP equivalent: [`GET /tags`](mnemos/user/http-api#tags)
...
- HTTP equivalent: `POST /tags/rename` (implemented in the API; not yet covered in [http-api.md](mnemos/user/http-api))
...
- CLI equivalent: [`mnemos add --url <URL>`](mnemos/user/cli-reference#add)
- HTTP equivalent: [`POST /memories` with manual content](mnemos/user/http-api#post-memories--create-memory)
- HTTP equivalent: [`POST /ingest-url`](mnemos/user/http-api#post-ingest-url--fetch-and-save-a-web-page)
- Security: [security.md](mnemos/admin/security#2-ssrf-prevention-memorymanager_validate_url)
...
- HTTP equivalent: [`POST /watch/start`](mnemos/user/http-api#post-watchstart--start-the-file-watcher)
...
- HTTP equivalent: [`POST /watch/stop`](mnemos/user/http-api#post-watchstop--stop-the-file-watcher)
...
- HTTP equivalent: [`GET /watch/status`](mnemos/user/http-api#get-watchstatus--watcher-status)
...
- HTTP equivalent: [`GET /auto-collect`](mnemos/user/http-api#get-auto-collect--compaction-signal-vector)
...
Same shape as the CLI `mnemos stats` command — see [cli-reference.md#stats](mnemos/user/cli-reference#stats).
...
- HTTP equivalent: [`GET /metrics`](mnemos/user/http-api#get-metrics)
- CLI equivalent: [`mnemos stats`](mnemos/user/cli-reference#stats)
...
- HTTP equivalent: [`POST /process`](mnemos/user/http-api#post-process--run-end-to-end-pipeline)
- CLI equivalent: [`mnemos processor run`](mnemos/user/cli-reference#processor)
...
- HTTP equivalent: [`POST /compress`](mnemos/user/http-api#post-compress--compress-content)
...
- HTTP equivalent: [`POST /retrieve`](mnemos/user/http-api#post-retrieve--retrieve-a-ccr-cached-original)
...
- Architecture: [overview.md#cachealigner-p1-5](mnemos/architecture/overview#cachealigner-p1-5)
- Config reference: [config.example.yaml](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/config.example.yaml)
...
| `profile` | string | no | auto-detected | Context Filter profile. See [context-filter.md#profiles](mnemos/user/context-filter#profiles). |
...
- [context-filter.md](mnemos/user/context-filter) — profiles, pipeline stages, auto-filter behaviour (the profile list lives there — not duplicated here)
- HTTP equivalent: [`POST /filter/{memory_id}`](mnemos/user/http-api#post-filtermemory_id--apply-the-5-stage-context-filter)
- CLI equivalent: [`mnemos filter`](mnemos/user/cli-reference#filter)
...
Optional scope tag (ADR-0027 Phase 0): `task:<slug>` (`[a-z0-9][a-z0-9\-_]{0,63}`, at most 1) narrows the entry to one task scope — see [tag-contract.md](mnemos/user/tag-contract#task--task-scope-multi-context-memory-adr-0027-phase-0).

Full reference: [tag-contract.md](mnemos/user/tag-contract).
...
- REST twin: `POST /context/assemble` (same manager path) — [http-api.md](mnemos/user/http-api)
...
- REST twin: `POST /context/rewrite` (same manager path) — [http-api.md](mnemos/user/http-api)
...
- REST twin: `POST /hooks/{action}` — [http-api.md](mnemos/user/http-api)
- Programmatic surface: `MnemosSDK` ([integration-guide.md](mnemos/user/integration-guide))
...
- [http-api.md](mnemos/user/http-api) — the nested `/memories/{id}/workflow` REST endpoints
- [tag-contract.md](mnemos/user/tag-contract) — the append-only classification layer (distinct from this mutable lifecycle layer)
...
- [getting-started.md](mnemos/user/getting-started) — wiring `mcp.json` and the first call
- [http-api.md](mnemos/user/http-api) — the same capabilities over HTTP
- [cli-reference.md](mnemos/user/cli-reference) — the same capabilities over the CLI
- [tag-contract.md](mnemos/user/tag-contract) — M2 schema enforced by `mnemos_add`
- [security.md](mnemos/admin/security) — SSRF guard, secrets hygiene
- [architecture overview](mnemos/architecture/overview#mcp-server) — server lifecycle
