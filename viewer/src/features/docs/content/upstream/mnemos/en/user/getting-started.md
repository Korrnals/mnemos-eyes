...
> Curator: the complete first run of Mnemos — one-line install, first memory, first search, connecting an agent harness.
...
For higher-level context, see [architecture overview](mnemos/architecture/overview). For every CLI subcommand, see [cli-reference.md](mnemos/user/cli-reference). For every MCP tool, see [mcp-tools.md](mnemos/user/mcp-tools). For every HTTP endpoint, see [http-api.md](mnemos/user/http-api).
...
| Container one-liner | `… install.sh \| bash -s -- --container` — see [container-deployment.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/en/admin/runbooks/container-deployment.md) |
| From source (contributors) | `git clone https://github.com/vesmaro/vesmaro && cd vesmaro && uv venv && source .venv/bin/activate && uv pip install -e ".[dev]"` — see [CONTRIBUTING.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/CONTRIBUTING.md) |
...
Full guide: [container-deployment.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/en/admin/runbooks/container-deployment.md).
...
The default provider is `ollama` pointing at `http://localhost:11434`. See [config.example.yaml](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/config.example.yaml) for the full provider list.
...
The tag contract is documented in [tag-contract.md](mnemos/user/tag-contract). The short version: every memory needs **exactly one** `project:<slug>`, **exactly one** `agent:<slug>`, and **at least one** `mnemos:<subtype>` (e.g. `mnemos:learning`, `mnemos:bug-pattern`, `mnemos:decision`).
...
For programmatic access with more options (vector weight, raw content, tag filter), use the HTTP API — see [http-api.md#search](mnemos/user/http-api#search).
...
| Anything else | [adapter-template.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/integrations/adapter-template.md) |

**The full copy-paste instructions for every harness live on one page: [Connect Mnemos to any harness](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/integrations/mcp-presets.md).** The behavioral layer — instructions, skills, and prompt mode that make agents actually *use* the memory — is a separate one-pass step:

```bash
mnemos integration setup
```

See the [integration guide](mnemos/user/integration-guide) for targets and flags.
...
> **Tip — auto-collect mode.** Set `MNEMOS_AUTO_COLLECT=1` in the server's `env` block to make Mnemos nudge your agent to call `mnemos_save_context` every ~6 tool calls. See [mcp-tools.md#auto-collect-mode](mnemos/user/mcp-tools#auto-collect-mode) for the trade-offs.
...
> **Security.** The default bind is `127.0.0.1`. Do not expose this port without a reverse proxy with authentication in front — see [security.md](mnemos/admin/security).
...
To run the full development gate (contributors only): clone the repo, `uv pip install -e ".[dev,mcp]"`, then `make verify` — ruff + mypy `--strict` + bandit + pip-audit + the test suite. If `pip-audit` complains about a pinned CVE, see the [dependency-updates runbook](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/en/admin/runbooks/dependency-updates.md).
...
Mnemos reads `config.yaml` from the current directory or `~/.mnemos/config.yaml`. See [config.example.yaml](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/config.example.yaml) for the full schema. The most useful knobs:
...
| If you want to… | Read |
|-----------------|------|
| Connect a specific harness (VS Code, Claude Code, Cursor, OpenCode, Codex, Windsurf, pi, Hermes…) | [Connect Mnemos to any harness](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/integrations/mcp-presets.md) |
| Deploy the behavioral pack (instructions / skills / prompts / agent wiring) | [integration-guide.md](mnemos/user/integration-guide) |
| See every CLI subcommand | [cli-reference.md](mnemos/user/cli-reference) |
| See every MCP tool | [mcp-tools.md](mnemos/user/mcp-tools) |
| See every HTTP endpoint | [http-api.md](mnemos/user/http-api) |
| Understand the system shape | [architecture overview](mnemos/architecture/overview) |
| Read the tag schema | [tag-contract.md](mnemos/user/tag-contract) |
| Run an operational task | [admin/runbooks/install.md](mnemos/admin/runbooks/install) |
| Review security boundaries | [security.md](mnemos/admin/security) |
| See why a decision was made | [project/adr/](https://github.com/vesmaro/vesmaro/tree/23f0fce42ea87c329d4049397292b3dca948773c/docs/project/adr) |
