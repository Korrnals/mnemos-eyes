---
title: Mnemos — Security Posture (M15.2)
...
> Curator: the Mnemos security posture — TOTP, non-loopback guard, threat model; background for the board's tokens page.
...
See [Tag Contract — `mnemos:no-federate`](mnemos/user/tag-contract#mnemosno-federate--federation-exclusion-marker)
...
[Federation — Batch Sync](mnemos/user/sync) for the operator guide.
