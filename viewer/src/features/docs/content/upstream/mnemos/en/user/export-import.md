...
> Curator: backup, move, and restore Mnemos memories via CLI or HTTP API — JSON with metadata or full SQLite snapshots.
...
- [CLI Reference](mnemos/user/cli-reference) — all `mnemos` subcommands.
- [HTTP API Reference](mnemos/user/http-api) — every endpoint.
- [Security Model](mnemos/admin/security) — encryption, secrets hygiene.
