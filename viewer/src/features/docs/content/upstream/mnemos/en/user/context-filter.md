...
> Curator: the five-stage pipeline cleaning raw content before a model sees it — what the board shows as «raw → processed».
...
- [Integration Guide](mnemos/user/integration-guide) — behavioural instructions that tell agents *when* to filter.
- [MCP Tools Reference](mnemos/user/mcp-tools) — full `mnemos_*` tool catalogue.
- [CLI Reference](mnemos/user/cli-reference) — every `mnemos` subcommand.
