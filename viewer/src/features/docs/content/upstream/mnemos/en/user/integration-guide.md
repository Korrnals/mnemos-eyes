...
> Curator: behavioral triggers that make agents actually use the memory — session-start recall, checkpoints, mandatory tags.
...
configuration, see [context-filter.md](mnemos/user/context-filter).
...
- **Lifecycle hooks** — the grouped `mnemos_hooks` MCP tool and the REST
  ...
  hook call. Full reference: [mcp-tools.md → `mnemos_hooks`](mnemos/user/mcp-tools#mnemos_hooks)
  / [http-api.md → Lifecycle hooks](mnemos/user/http-api).
...
the knowledge pipeline — see the [Context Filter](mnemos/user/context-filter) and
[architecture overview](mnemos/architecture/overview) for the pipeline
stages.
...
For VS Code Copilot Chat, see [getting-started.md](mnemos/user/getting-started#connect-your-harness-mcp)
...
[`integrations/mcp-presets.md`](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/integrations/mcp-presets.md).
...
See [tag-contract.md](mnemos/user/tag-contract) for the full schema. The integration
