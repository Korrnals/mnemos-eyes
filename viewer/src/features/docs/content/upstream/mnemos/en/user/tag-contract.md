...
> Curator: the mandatory tag schema of Mnemos — prefixes, allowed values, migration; the board's «Mnemos tags» field follows it.
...
3. Check the contract with `mnemos tags validate` (the full vault scan is
   ...
   meanwhile, see [cli-reference.md](mnemos/user/cli-reference#tags-validate)).
