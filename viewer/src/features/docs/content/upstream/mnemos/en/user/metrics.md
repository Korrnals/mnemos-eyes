...
> Curator: the three HTTP endpoints behind the board's «Пульс» feed and overview stats, plus a Prometheus exposition.
...
- [HTTP API Reference](mnemos/user/http-api) — every endpoint.
- [CLI Reference](mnemos/user/cli-reference) — `mnemos stats`, `mnemos logs`.
- [Context Filter](mnemos/user/context-filter) — the `filter` section in stats.
