---
title: Federation — Batch Sync (Phase 0)
...
> Curator: offline batch sync between two Mnemos instances via cron and rsync — Mnemos itself makes no network calls.
...
[Tag Contract — `mnemos:no-federate`](mnemos/user/tag-contract#mnemosno-federate--federation-exclusion-marker)
...
secret. See [Security — Federation defence-in-depth](mnemos/admin/security#11-federation-defence-in-depth).

---

## See also

- [Export & Import](mnemos/user/export-import) — full backups (JSON / SQLite).
- [Security — Federation defence-in-depth](mnemos/admin/security#11-federation-defence-in-depth) — the three-layer model.
- [Tag Contract — `mnemos:no-federate`](mnemos/user/tag-contract#mnemosno-federate--federation-exclusion-marker) — the exclusion marker.
- [MCP Tools](mnemos/user/mcp-tools) — `mnemos_export` / `mnemos_import` MCP tools (the MCP surface for full export/import).
