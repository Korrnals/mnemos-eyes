...
> Curator: the full HTTP surface of Mnemos — memory, search, pipeline, DLQ, traces, A2A sessions; the requests the board makes to stores.
...
> **Default bind is `127.0.0.1`.** Do not expose this port to a public network without putting a reverse proxy with authentication in front. See [security.md](mnemos/admin/security) for the threat model.
...
For the same capabilities over other transports, see [mcp-tools.md](mnemos/user/mcp-tools) (MCP) and [cli-reference.md](mnemos/user/cli-reference) (CLI). For higher-level context, see [architecture overview](mnemos/architecture/overview). The A2A contract is also documented in [a2a-sessions.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/en/architecture/a2a-sessions.md) (link to the design rationale).
...
**Request body** — see [MemoryCreate](mnemos/architecture/overview#data-model)
...
Full field-by-field documentation: [`mcp-tools.md` → `mnemos_assemble_context`](mnemos/user/mcp-tools#mnemos_assemble_context).
...
Full field-by-field documentation: [`mcp-tools.md` → `mnemos_context_rewrite`](mnemos/user/mcp-tools#mnemos_context_rewrite).
...
field-by-field documentation: [`mcp-tools.md` → `mnemos_hooks`](mnemos/user/mcp-tools#mnemos_hooks).
...
> Mounted under `/v1`. The contract is described in detail in [a2a-sessions.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/en/architecture/a2a-sessions.md); this section is the HTTP surface.
...
- [mcp-tools.md](mnemos/user/mcp-tools) — same capabilities over MCP
- [cli-reference.md](mnemos/user/cli-reference) — same capabilities over the CLI
- [architecture overview](mnemos/architecture/overview) — system shape and data model
- [a2a-sessions.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/en/architecture/a2a-sessions.md) — A2A contract and design rationale
- [tag-contract.md](mnemos/user/tag-contract) — M2 schema enforced by `POST /memories`
- [security.md](mnemos/admin/security) — SSRF guard, secrets hygiene, auth model, request-boundary rules
