...
> Curator: complete reference for the mnemos CLI — every subcommand with runnable examples.

# CLI Reference

> Complete reference for the `mnemos` command-line tool.

The CLI is a thin Typer-based wrapper around [`MemoryManager`](mnemos/architecture/overview#memorymanager). It uses Rich for table / colour output and is the most convenient way to interact with Mnemos from a shell.
...
For a step-by-step first run, see [getting-started.md](mnemos/user/getting-started). For programmatic access, see [mcp-tools.md](mnemos/user/mcp-tools) and [http-api.md](mnemos/user/http-api).
...
| [`integration`](mnemos/user/integration-guide) | Deploy / verify the integration layer (dedicated page) |
...
| [`export`](mnemos/user/export-import) | Export memories to a JSON / SQLite backup (dedicated page) |
| [`import`](mnemos/user/export-import) | Import memories from an export file (dedicated page) |
| [`logs`](#logs) | View pipeline traces |
| [`sync`](mnemos/user/sync) | Federation batch sync export / import (dedicated page) |
...
> **Tag contract.** Every entry must have `project:<slug>`, `agent:<slug>`, and at least one `mnemos:<subtype>`. The CLI enforces this in strict mode (the default). See [tag-contract.md](mnemos/user/tag-contract) for the full schema.
...
For richer query power over HTTP, use the API `POST /search` (see [http-api.md#search](mnemos/user/http-api#search)).
...
When `--agent` is passed **without** a query, the result is the N most recent entries for that agent, ordered by `created_at desc`. This is the same data the MCP tool [`mnemos_agent_recall`](mnemos/user/mcp-tools#mnemos_agent_recall) returns.
...
- MCP tool: [`mnemos_workflow`](mnemos/user/mcp-tools#mnemos_workflow)
...
- [context-filter.md](mnemos/user/context-filter) — profiles, pipeline stages, auto-filter behaviour
- MCP tool: [`mnemos_filter`](mnemos/user/mcp-tools#mnemos_filter)
...
> **Security.** The default bind is `127.0.0.1`. Do not expose this port to a public network without putting a reverse proxy with authentication in front. See [security.md](mnemos/admin/security).
...
The full HTTP API surface is documented in [http-api.md](mnemos/user/http-api). The Swagger UI is served at `http://HOST:PORT/docs`.
...
See [mcp-tools.md](mnemos/user/mcp-tools) for the full tool list and [getting-started.md#run-the-mcp-server](mnemos/user/getting-started#connect-your-harness-mcp) for the VS Code wiring.
...
- HTTP equivalent: [`GET /traces`](mnemos/user/http-api#get-traces--list-pipeline-traces)
...
- [sync.md](mnemos/user/sync#mnemosno-federate-exclusion) — what `mnemos:no-federate` excludes
...
- [getting-started.md](mnemos/user/getting-started) — first-run walkthrough
- [mcp-tools.md](mnemos/user/mcp-tools) — the same capabilities exposed over MCP
- [http-api.md](mnemos/user/http-api) — the same capabilities exposed over HTTP
- [context-filter.md](mnemos/user/context-filter) — filter profiles used by `add --dry-run` and `filter`
- [tag-contract.md](mnemos/user/tag-contract) — the tag schema enforced here
- [runbooks/migrate.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/en/admin/runbooks/migrate.md) — operational migration guide
- [architecture overview](mnemos/architecture/overview) — system shape
