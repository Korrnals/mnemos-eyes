---
title: Справочник MCP-инструментов
slug: mnemos/user/mcp-tools
...
> Куратор: все инструменты mnemos_* для MCP-клиентов — тот же слой, через который агенты работают с памятью на борде.
...
Быстрое подключение к VS Code — в [getting-started.md#run-the-mcp-server](mnemos/user/getting-started#подключите-ваш-харнес-mcp). Те же возможности доступны через HTTP — см. [http-api.md](mnemos/user/http-api). Схема тегов, соблюдаемая большинством инструментов, — в [tag-contract.md](mnemos/user/tag-contract).
...
Создать новую запись в памяти. MCP-слой применяет контракт тегов Mnemos ([M2](mnemos/user/tag-contract)) перед записью.
...
| `❌ Error: ...` | Сбой записи SQLite, vault или ошибка эмбеддинга (последняя не критична — см. [обзор архитектуры](mnemos/architecture/overview#1-хранилище-storage-layer)). |

### Связанные ресурсы

- Схема тегов: [tag-contract.md](mnemos/user/tag-contract)
- HTTP-эквивалент: [`POST /memories`](mnemos/user/http-api#post-memories--создать-запись-create-memory)
- CLI-эквивалент: [`mnemos add`](mnemos/user/cli-reference#add)
...
- HTTP-эквивалент: [`POST /search`](mnemos/user/http-api#post-search--гибридный-поиск)
- CLI-эквивалент: [`mnemos search`](mnemos/user/cli-reference#search)
...
- HTTP-эквивалент: [`GET /recall/agent/{name}`](mnemos/user/http-api#get-recallagentname--отзыв-агента)
- CLI-эквивалент: [`mnemos recall --agent <slug>`](mnemos/user/cli-reference#recall)
...
- [architecture.md](mnemos/architecture/overview)
- HTTP-эквивалент: [`POST /context/recall`](mnemos/user/http-api#post-contextrecall--отозвать-контекст-сессии)
...
- HTTP-эквивалент: [`POST /context/save`](mnemos/user/http-api#post-contextsave--сохранить-чекпойнт-сессии)
...
- HTTP-эквивалент: [`GET /memories`](mnemos/user/http-api#get-memories--список-последних)
- CLI-эквивалент: [`mnemos recall`](mnemos/user/cli-reference#recall)
...
- HTTP-эквивалент: [`GET /tags`](mnemos/user/http-api#get-tags--список-тегов-с-количеством)
...
- HTTP-эквивалент: `POST /tags/rename` (реализован в API; пока не описан в [http-api.md](mnemos/user/http-api))
...
- CLI-эквивалент: [`mnemos add --url <URL>`](mnemos/user/cli-reference#add)
- HTTP-эквивалент: [`POST /memories` с ручным контентом](mnemos/user/http-api#post-memories--создать-запись-create-memory)
- HTTP-эквивалент: [`POST /ingest-url`](mnemos/user/http-api#post-ingest-url--получить-и-сохранить-веб-страницу)
- Безопасность: [security.md](mnemos/admin/security#2-защита-от-ssrf-memorymanager_validate_url)
...
- HTTP-эквивалент: [`POST /watch/start`](mnemos/user/http-api#post-watchstart--запустить-файловый-наблюдатель)
...
- HTTP-эквивалент: [`POST /watch/stop`](mnemos/user/http-api#post-watchstop--остановить-файловый-наблюдатель)
...
- HTTP-эквивалент: [`GET /watch/status`](mnemos/user/http-api#get-watchstatus--статус-наблюдателя)
...
- HTTP-эквивалент: [`GET /auto-collect`](mnemos/user/http-api#get-auto-collect--вектор-сигналов-компакции)
...
Та же структура, что и у команды CLI `mnemos stats` — см. [cli-reference.md#stats](mnemos/user/cli-reference#stats).
...
- HTTP-эквивалент: [`GET /metrics`](mnemos/user/http-api#get-metrics)
- CLI-эквивалент: [`mnemos stats`](mnemos/user/cli-reference#stats)
...
- HTTP-эквивалент: [`POST /process`](mnemos/user/http-api#post-process--запустить-end-to-end-пайплайн)
- CLI-эквивалент: [`mnemos processor run`](mnemos/user/cli-reference#processor)
...
- HTTP-эквивалент: [`POST /compress`](mnemos/user/http-api#post-compress--сжать-контент)
...
- HTTP-эквивалент: [`POST /retrieve`](mnemos/user/http-api#post-retrieve--извлечь-оригинал-из-ccr-кэша)
...
- Архитектура: [overview.md#cachealigner-p1-5](mnemos/architecture/overview#cachealigner-p1-5)
- Референс конфига: [config.example.yaml](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/config.example.yaml)
...
| `profile` | string | нет | автоопределение | Профиль контекстного фильтра. См. [context-filter.md#профили](mnemos/user/context-filter#профили). |
...
- [context-filter.md](mnemos/user/context-filter) — профили, этапы конвейера, поведение автофильтра (список профилей живёт там — здесь не дублируется)
- HTTP-эквивалент: [`POST /filter/{memory_id}`](mnemos/user/http-api#post-filtermemory_id--применить-5-этапный-контекстный-фильтр)
- CLI-эквивалент: [`mnemos filter`](mnemos/user/cli-reference#filter)
...
Опциональный скоуп-тег (ADR-0027 Фаза 0): `task:<slug>` (`[a-z0-9][a-z0-9\-_]{0,63}`, не более 1) сужает запись до одной task-области — см. [tag-contract.md](mnemos/user/tag-contract#task--task-область-многоконтекстная-память-adr-0027-фаза-0).

Полная справка: [tag-contract.md](mnemos/user/tag-contract).
...
- REST-близнец: `POST /context/assemble` (тот же путь менеджера) — [http-api.md](mnemos/user/http-api)
...
- REST-близнец: `POST /context/rewrite` (тот же путь менеджера) — [http-api.md](mnemos/user/http-api)
...
- REST-близнец: `POST /hooks/{action}` — [http-api.md](mnemos/user/http-api)
- Программная поверхность: `MnemosSDK` ([integration-guide.md](mnemos/user/integration-guide))
...
- [http-api.md](mnemos/user/http-api) — вложенные REST-эндпоинты `/memories/{id}/workflow`
- [tag-contract.md](mnemos/user/tag-contract) — слой append-only классификации (отличается от этого изменяемого слоя жизненного цикла)
...
- [getting-started.md](mnemos/user/getting-started) — подключение `mcp.json` и первый вызов
- [http-api.md](mnemos/user/http-api) — те же возможности через HTTP
- [cli-reference.md](mnemos/user/cli-reference) — те же возможности через CLI
- [tag-contract.md](mnemos/user/tag-contract) — схема M2, соблюдаемая `mnemos_add`
- [security.md](mnemos/admin/security) — защита от SSRF, безопасность секретов
- [обзор архитектуры](mnemos/architecture/overview#mcp-сервер) — жизненный цикл сервера
