---
title: Mnemos — Состояние безопасности (M15.2)
slug: mnemos/admin/security
...
> Куратор: позиция безопасности mnemos — TOTP, не-loopback guard, модель угроз; фон для страницы борда «Токены и доступ».
...
См. [Tag Contract — `mnemos:no-federate`](mnemos/user/tag-contract#mnemosno-federate--маркер-исключения-из-федерации)
...
[Федерация — пакетная синхронизация](mnemos/user/sync) для руководства
оператора.
