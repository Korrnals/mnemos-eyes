---
title: Федерация — пакетная синхронизация (Phase 0)
...
> Куратор: офлайн-синхронизация двух инстансов mnemos по cron и rsync — mnemos сам сетевых вызовов не делает.
...
[Tag Contract — `mnemos:no-federate`](mnemos/user/tag-contract#mnemosno-federate--маркер-исключения-из-федерации)
...
секрет. См. [Security — Federation defence-in-depth](mnemos/admin/security#11-federation-defence-in-depth).

---

## См. также

- [Export & Import](mnemos/user/export-import) — полные бэкапы (JSON / SQLite).
- [Security — Federation defence-in-depth](mnemos/admin/security#11-federation-defence-in-depth) — трёхслойная модель.
- [Tag Contract — `mnemos:no-federate`](mnemos/user/tag-contract#mnemosno-federate--маркер-исключения-из-федерации) — маркер исключения.
- [MCP Tools](mnemos/user/mcp-tools) — `mnemos_export` / `mnemos_import` MCP-инструменты (MCP-поверхность для полного export/import).
