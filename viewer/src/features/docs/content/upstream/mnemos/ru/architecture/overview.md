---
title: Mnemos — Архитектура системы
slug: mnemos/architecture/overview
category: mnemos-architecture
order: 1
...
> Куратор: как устроен mnemos — гибридная память, поверхности CLI/HTTP/MCP и место vesmaro-eyes в картине.
...
MCP-сервер и Obsidian-совместимый vault. Web UI для Mnemos — это **vesmaro-eyes**, борд, на котором вы читаете эту документацию.
...
```mermaid
flowchart LR
    A[Сырой текст] --> B[Сжатие<br/>5-ступенчатый фильтр]
    B --> C[Кэш оригинала<br/>по SHA-256]
    C --> D[Маркер<br/>в сжатый вывод]
    D --> E[Сжатый текст<br/>+ маркер]
    F[mnemos_retrieve<br/>хеш] --> G{query?}
    G -->|нет| H[Полный оригинал]
    G -->|да| I[FTS5-сниппеты<br/>ранжированные]
    C -.-> H
    C -.-> I
```
...
```mermaid
flowchart LR
    A[System-prompt текст] --> B[Извлечь динамические спаны<br/>упорядочены по специфичности]
    B --> C[Удалить спаны из тела<br/>схлопнуть пробелы]
    C --> D[Добавить блок '--- Dynamic context ---'<br/>со строками kind: value]
    D --> E[Выровненный текст<br/>побайтово стабильный prefix]
```
...
Когда `cache_aligner.enabled` равно `false`, `align_prefix()` возвращает текст без изменений с пустым списком `extracted`. MCP-инструмент `mnemos_align_prefix` — публичная поверхность; см. [mcp-tools.md#mnemos_align_prefix](mnemos/user/mcp-tools#mnemos_align_prefix).
...
См. [mcp-tools.md#сокращение-токенов-вывода-p1-7](mnemos/user/mcp-tools#сокращение-токенов-вывода-p1-7) — пользовательский референс.
...
#### MCP-сервер
Инструменты для Copilot/LLM-агентов (полный каталог — в [mcp-tools.md](mnemos/user/mcp-tools)):
...
> [PLAN.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/PLAN.md).
