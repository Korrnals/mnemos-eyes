...
> Куратор: полная HTTP-поверхность mnemos — память, поиск, конвейер, DLQ, трассировки, A2A-сессии; именно этими запросами борд читает хранилища.
...
> **Привязка по умолчанию — `127.0.0.1`.** Не открывайте этот порт в публичную сеть без обратного прокси с аутентификацией. Модель угроз — в [security.md](mnemos/admin/security).
...
Те же возможности через другие транспорты — в [mcp-tools.md](mnemos/user/mcp-tools) (MCP) и [cli-reference.md](mnemos/user/cli-reference) (CLI). Для общего контекста — [обзор архитектуры](mnemos/architecture/overview). Контракт A2A также описан в [a2a-sessions.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/ru/architecture/a2a-sessions.md) (ссылка на обоснование дизайна).
...
**Тело запроса** — см. [MemoryCreate](mnemos/architecture/overview#структура-данных)
...
Полная документация по полям: [`mcp-tools.md` → `mnemos_assemble_context`](mnemos/user/mcp-tools#mnemos_assemble_context).
...
Полная документация по полям: [`mcp-tools.md` → `mnemos_context_rewrite`](mnemos/user/mcp-tools#mnemos_context_rewrite).
...
[`mcp-tools.md` → `mnemos_hooks`](mnemos/user/mcp-tools#mnemos_hooks).
...
> Смонтировано под `/v1`. Контракт подробно описан в [a2a-sessions.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/ru/architecture/a2a-sessions.md); этот раздел — HTTP-поверхность.
...
- [обзор архитектуры](mnemos/architecture/overview) — структура системы и модель данных
- [a2a-sessions.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/ru/architecture/a2a-sessions.md) — контракт A2A и обоснование дизайна
- [tag-contract.md](mnemos/user/tag-contract) — схема M2, соблюдаемая `POST /memories`
- [security.md](mnemos/admin/security) — защита от SSRF, безопасность секретов, модель аутентификации, правила на границе запросов
