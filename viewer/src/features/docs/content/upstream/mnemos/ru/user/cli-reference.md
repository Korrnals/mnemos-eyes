...
slug: mnemos/user/cli-reference
...
> Куратор: полный справочник CLI mnemos — все подкоманды с запускабельными примерами.

# Справочник CLI

> Полная справка по командной строке `mnemos`.

CLI — тонкая обёртка на Typer вокруг [`MemoryManager`](mnemos/architecture/overview#memorymanager). Использует Rich для вывода таблиц с цветами и является наиболее удобным способом работы с Mnemos из оболочки.
...
Пошаговое первое использование — в [getting-started.md](mnemos/user/getting-started). Для программного доступа — [mcp-tools.md](mnemos/user/mcp-tools) и [http-api.md](mnemos/user/http-api).
...
| [`integration`](mnemos/user/integration-guide) | Развёртывание / проверка слоя интеграции (отдельная страница) |
...
| [`export`](mnemos/user/export-import) | Экспорт записей в JSON / SQLite-бэкап (отдельная страница) |
| [`import`](mnemos/user/export-import) | Импорт записей из файла экспорта (отдельная страница) |
| [`logs`](#logs) | Просмотр трассировок пайплайна |
| [`sync`](mnemos/user/sync) | Пакетная federation-синхронизация: export / import (отдельная страница) |
...
> **Контракт тегов.** Каждая запись должна иметь `project:<slug>`, `agent:<slug>` и хотя бы один `mnemos:<subtype>`. CLI соблюдает это в strict-режиме (по умолчанию). Полная схема — в [tag-contract.md](mnemos/user/tag-contract).
...
Для более широких возможностей запросов используйте HTTP API `POST /search` (см. [http-api.md#search](mnemos/user/http-api#post-search--гибридный-поиск)).
...
Когда `--agent` передан **без** запроса, результат — N последних записей этого агента, упорядоченных по `created_at desc`. Это те же данные, которые возвращает MCP-инструмент [`mnemos_agent_recall`](mnemos/user/mcp-tools#mnemos_agent_recall).
...
- MCP-инструмент: [`mnemos_workflow`](mnemos/user/mcp-tools#mnemos_workflow)
...
- [context-filter.md](mnemos/user/context-filter) — профили, этапы конвейера, автофильтр
- MCP-инструмент: [`mnemos_filter`](mnemos/user/mcp-tools#mnemos_filter)
...
> **Безопасность.** Привязка по умолчанию — `127.0.0.1`. Не открывайте этот порт в публичную сеть без обратного прокси с аутентификацией. См. [security.md](mnemos/admin/security).
...
Полная поверхность HTTP API документирована в [http-api.md](mnemos/user/http-api). Swagger UI доступен по адресу `http://HOST:PORT/docs`.
...
Полный список инструментов — в [mcp-tools.md](mnemos/user/mcp-tools), подключение к VS Code — в [getting-started.md#run-the-mcp-server](mnemos/user/getting-started#подключите-ваш-харнес-mcp).
...
- HTTP-эквивалент: [`GET /traces`](mnemos/user/http-api#get-traces--список-трассировок-пайплайна)
...
- [sync.md](mnemos/user/sync#исключение-mnemosno-federate) — что исключает `mnemos:no-federate`
...
- [getting-started.md](mnemos/user/getting-started) — первое использование
- [mcp-tools.md](mnemos/user/mcp-tools) — те же возможности через MCP
- [http-api.md](mnemos/user/http-api) — те же возможности через HTTP
- [context-filter.md](mnemos/user/context-filter) — профили фильтра, используемые `add --dry-run` и `filter`
- [tag-contract.md](mnemos/user/tag-contract) — схема тегов, соблюдаемая здесь
- [runbooks/migrate.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/ru/admin/runbooks/migrate.md) — операционное руководство по миграции
- [обзор архитектуры](mnemos/architecture/overview) — структура системы
