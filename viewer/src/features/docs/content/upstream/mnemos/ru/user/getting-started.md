...
> Куратор: полный первый прогон mnemos — установка одной командой, первая запись, первый поиск, подключение агента-харнеса.
...
Для общего контекста см. [обзор архитектуры](mnemos/architecture/overview). Справочник
по всем подкомандам CLI — [cli-reference.md](mnemos/user/cli-reference). По каждому
MCP-инструменту — [mcp-tools.md](mnemos/user/mcp-tools). По каждому HTTP-эндпоинту —
[http-api.md](mnemos/user/http-api).
...
| Контейнер одной командой | `… install.sh \| bash -s -- --container` — см. [container-deployment.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/ru/admin/runbooks/container-deployment.md) |
| Из исходников (контрибьюторам) | `git clone https://github.com/vesmaro/vesmaro && cd vesmaro && uv venv && source .venv/bin/activate && uv pip install -e ".[dev]"` — см. [CONTRIBUTING.ru.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/CONTRIBUTING.ru.md) |
...
Полное руководство: [container-deployment.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/ru/admin/runbooks/container-deployment.md).
...
Полную матрицу провайдеров см. в [config.example.yaml](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/config.example.yaml).
...
Контракт тегов описан в [tag-contract.md](mnemos/user/tag-contract). Коротко: каждая запись требует
...
по тегам) используйте HTTP API — см. [http-api.md#search](mnemos/user/http-api#post-search--гибридный-поиск).
...
| Всё остальное | [adapter-template.md](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/integrations/adapter-template.md) |

**Полные инструкции для копирования для каждого харнесса собраны на одной странице:
[Подключите Mnemos к любому харнесу](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/integrations/mcp-presets.md).** Поведенческий
...
Таргеты и флаги — в [руководстве по интеграции](mnemos/user/integration-guide).
...
> вызовов инструментов. О компромиссах см. [mcp-tools.md#auto-collect-mode](mnemos/user/mcp-tools#режим-auto-collect).
...
> порт наружу без обратного прокси с аутентификацией — см. [security.md](mnemos/admin/security).
...
см. [ранбук по обновлению зависимостей](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/docs/ru/admin/runbooks/dependency-updates.md).
...
Полная схема — в [config.example.yaml](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/config.example.yaml). Самые полезные ручки:
...
«один владелец на хранилище» в [руководстве по интеграции](mnemos/user/integration-guide).
...
| Если хотите… | Читайте |
|--------------|---------|
| Подключить конкретный харнес (VS Code, Claude Code, Cursor, OpenCode, Codex, Windsurf, pi, Hermes…) | [Подключите Mnemos к любому харнесу](https://github.com/vesmaro/vesmaro/blob/23f0fce42ea87c329d4049397292b3dca948773c/integrations/mcp-presets.md) |
| Развернуть поведенческий пакет (инструкции / скиллы / промпты / wiring агентов) | [integration-guide.md](mnemos/user/integration-guide) |
| Посмотреть все подкоманды CLI | [cli-reference.md](mnemos/user/cli-reference) |
| Посмотреть все MCP-инструменты | [mcp-tools.md](mnemos/user/mcp-tools) |
| Посмотреть все HTTP-эндпоинты | [http-api.md](mnemos/user/http-api) |
| Понять устройство системы | [обзор архитектуры](mnemos/architecture/overview) |
| Прочитать схему тегов | [tag-contract.md](mnemos/user/tag-contract) |
| Выполнить операционную задачу | [admin/runbooks/install.md](mnemos/admin/runbooks/install) |
| Пересмотреть границы безопасности | [security.md](mnemos/admin/security) |
| Узнать, почему принято то или иное решение | [project/adr/](https://github.com/vesmaro/vesmaro/tree/23f0fce42ea87c329d4049397292b3dca948773c/docs/project/adr) |
