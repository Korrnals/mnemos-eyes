---
title: Контракт тегов
slug: mnemos/user/tag-contract
...
> Куратор: обязательная схема тегов mnemos — префиксы, допустимые значения, миграция; поле «Теги mnemos» у задач борда живёт по этому контракту.
...
3. Проверьте контракт командой `mnemos tags validate` (полный обход vault
   ...
   или `GET /memories`, см. [cli-reference.md](mnemos/user/cli-reference#tags-validate)).
