...
> Куратор: три HTTP-эндпоинта, на которых держатся «Пульс» и статистика «Обзора» борда, плюс Prometheus-экспозиция.
...
- [Справочник CLI](mnemos/user/cli-reference) — `mnemos stats`, `mnemos logs`.
- [Контекстный фильтр](mnemos/user/context-filter) — секция `filter` в статистике.
