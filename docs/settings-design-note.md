...
- Status: **design only — no implementation in this slice**. Owner ask:
  "important settings for toggling components and modes — thoughtfully,
  not blindly everything." Branch `docs/settings-design`, not for merge
  until owner + TL review.
...
lives in the **React viewer (L1)** and was built as a shell ("further
sections join as sibling blocks" — `viewer/src/app/routes.tsx`). Expanding
it is normal L1 work: **no freeze-exception entry needed**; `web/` untouched.

## 1. Settings architecture today (facts)

- Storage: `board_meta` KV in SQLite (`store.get/set_meta`).
- Validation: server-side 422 gates (`_validate_default_executor`
  precedent); the client mirrors gate reasons verbatim, never invents.
- AuthZ: writes UI-token gated (`_guard_ui_write`); secrets never rendered.
- Audit: `store.log_board_event` → SSE `/api/events` — a change-history
  feed already exists (`default.changed`, `automation.settings.changed`,
  `mesh.node.changed`). No new plumbing for history.
- Live surfaces: execution defaults (page), automation kill-switch + cap
  (API; engine honestly inert in scheduler-S1), store enable/disable/pause
  (`/api/memory/servers/{name}/action`, UI-gated).
...
Assignment test, in order: (1) board-own read/render/pacing only → **A**.
(2) Changes runtime behavior of board-owned components, or forwards a
node-affecting intent → **B**. (3) Trust, data topology, credentials, or
irreversible surface → **C**.
...
| Setting | Flavor | Gate |
| --- | --- | --- |
| Automation kill switch ON | board | confirm modal; load-bearing only when scheduler-S2 engine lands |
...
| Mesh-node observation on/off | board | edits node registry; UI is deliberately read-only today — lift only by explicit decision (phase 2) |
| Explicit one-shot sync / lazy-fetch trigger | **intent** | ONE intent to operator's own mnemos (Q10.1); mnemos confirm-gate owns the decision; typed confirm + gate result surfaced |
...
- Certs/CA/mTLS, ACL & moderation policy, DB migrations, poller allowlist
  (laptop config), S3 join admission (user-confirmed on the receiving
  store, fail closed), signed-origins v2.

Vocabulary guard: scheduler S1/S2 (ADR 0013) ≠ data-topology S1/S2/S3
(ADR-0021). The page must label which "mode" it means, always.

## 3. Architecture principle — observe, don't command (ROADMAP-v2 §8)

- The board does not become a control plane: **no new write path from this
  page to mesh nodes is created**, ever. Tier B splits into:
...
- Modes are read, not written: the board shows each node's effective mode
  and sync watermark; switching stays in config with review.
...
2. mesh/mnemos: healthz self-report — effective federation mode,
   ...
   fields; powers the Federation display section.
...
- Sections in tier order: «Display & observation» (A) → «Behavior» (B) →
  «Federation» (read-only state; sync trigger appears only when API 3
  lands) → footer «CLI/API-only» list for Tier C (names + pointers, no
  controls).
- Search box filters across all sections (label + description match).
- Change history: collapsible panel fed by the existing events feed
  filtered to settings/change kinds; per-setting "last changed" hint.
- Tier B confirm modal states the consequence in one line («queries stop
  ...
  typed confirm and surfaces the mnemos gate verdict.
- Honest inert labels wherever behavior is inert today (automation-engine
  precedent).
...
6. Store quick toggles enable/disable/pause (existing action API + confirm).
7. Viewer locale + theme.
8. Federation display column (effective mode + watermark) — lands with
   healthz self-report; display only.

Deliberately NOT in phase 1: node-observation toggles in UI (API-only
posture stands), any sync-trigger button (waits for the mnemos intent
API), every Tier C item.

## 7. Not-doing

No mesh control surface, no mode switching, no credential editing, no
per-project executor overrides (reserved scope) in this page's first two
phases. Implementation follows as separate slices after owner ratification;
any §8 posture change goes through archcom first.
