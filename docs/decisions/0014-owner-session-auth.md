...
- Status: **Accepted** (комитет 2026-09-22; ратифицировано владельцем
  2026-09-22 с поправкой по TTL: **скользящий idle-таймаут 6 часов** —
  «если никаких действий в интерфейсе за это время не происходит —
  разлогинивать»; дословная улика в протоколе комитета)
...
- Ставится успехом verify: `Set-Cookie: vesmaro_ui=<token>; HttpOnly;
  ...
  петлю на http). **TTL — скользящий idle-таймаут 6 часов** (поправка
  владельца при ратификации): cookie ставится/перевыпускается с
  `Max-Age=21600` на каждом успешном авторизованном запросе (ui-guard
  пропустил → свежий `Set-Cookie`; перевыпускать можно не чаще раза в
  N минут, чтобы не шуметь). 6 часов тишины → браузер гасит cookie сам →
  следующая мутация получает 401 с текстом «сессия истекла» (уже
  существующий rejectDetail-путь). Активность продлевает — ровно
  семантика владельца; серверного store сессий по-прежнему не нужно.
...
- CSRF: `SameSite=Strict` — основной барьер (state-changing запросы
  POST/DELETE, CORS не открывается); явная проверка `Origin` на
  `/api/auth/*` — отложена, триггер: появление браузеров, игнорирующих
  SameSite, или multi-origin сценарий; double-submit — overkill
  (ревью 2026-09-22: «проверка Origin» из черновика не реализована —
  зафиксировано честно, а не притворно).
...
- **Честные остаточные риски** (принятые):
  ...
  - idle > 6ч разлогинивает (поправка владельца); «запомнить меня
    навсегда» — out of scope

## Out of scope

Мультиюзер / SSO / PAKE; «запомнить меня» между рестартами браузера
(скользящий idle-таймаут 6ч переживает рестарт только в пределах
неистёкшего Max-Age — это следствие формы, не отдельная фича); управление
board-токеном в UI; Tauri-клиент — стаб, но построение «cookie ИЛИ
header» уже совместимо с custom-protocol fetch (заметка на Фазу-2,
сегодня риска нет).
