...
| **device** | спаренные устройства | v0 — read-only; **v1 (Amendment 2026-09-23): scope `control` — мутации задач/reports/inbox/notifications, §A ниже**; с §A.7 — права = глобальные read всегда + пер-устройственный набор гранул (`grants`); store-ops/валидация — никогда; DELETE задач, automation, agent-loop, конфигурация памяти — закрыто всегда | v0: sliding 30 д при активности; v1: control — sliding 7 д; **с §A.7 — единые sliding 30 д / hard 90 д для всех** (revocation-first) |
...
### A.7 Owner override (2026-09-23, фидбек по живому использованию): revocation-first + per-device grants

Дословно:

> «Наш подход с 6 часами (7-дневный слайд control) — не подходит! Надо
> иметь возможность просто отзывать доступ (вроде реализовано, но проверь
> что точно работает). Логика проста: устройство скомпрометировано —
> отзываем токен и всё. Но стоит этот кейс ещё раз хорошенько продумать
> и покумекать.»
>
> «В настройках — давать и забирать доступы к компонентам по
> подключенным устройствам — это безопасно и пользователь-администратор
> сам определяет, кому сколько и куда разрешений выдать и забрать.»

**TTL-ревизия.** Дифференцированный sliding (control 7 д / read 30 д)
выведен из эксплуатации: единые **sliding 30 д / hard 90 д** для всех
устройств. Обоснование: авто-срок не отвечает на вопрос владельца
«скомпрометировано — отзываем и всё»; окно 7 д не защищает от
активного злоумышленника (активность продлевает sliding), а честный
kill-switch уже есть — `DELETE /api/devices/{id}` + SSE
`pairing.revoked` + 401 на следующем же запросе. Security-риск
компенсируется связкой **ревок + атрибуция (`device:<id>` в task
history) + пер-устройственные гранулы** (взломанное устройство
ограничено выданными ему правами).

**Per-device grants (гранулы).** Права устройства = глобальные read
(таблица `_DEVICE_READ_ROUTES`, открыта каждому валидному токену
всегда) + пер-устройственный набор гранул муутаций. Гранулы v1 (словарь
`DEVICE_GRANTS`, расширяемый):

| Гранула | Роуты муутаций |
| --- | --- |
| `tasks` | `POST /api/tasks`, `PATCH /api/tasks/{id}`, `…/move`, `…/archive`, `…/unarchive` |
| `reports` | `POST /api/tasks/{id}/reports` |
| `inbox` | `POST /api/tasks/inbox/refresh`, `POST /api/tasks/inbox/{id}/adopt` |
| `notifications` | `POST /api/notifications/read` |

Hard-deny семейства (§A.1) закрыты независимо от гранул. Колонка
`scope` остаётся записью класса пейринга (read|control) для аудита и
mode-line — **guard'ом больше не читается**; единственный источник прав
— колонка `grants` (JSON-набор, '' = «не проинициализировано»,
'[]' = владелец забрал всё — миграция это множество никогда не
перезаполняет).

- **API**: `PUT /api/devices/{id}/grants` (ui-token, full-replacement,
  идемпотент) → 200 `{ok, device}`; 404 неизвестное устройство; 409
  не-active сессия; 422 неизвестная гранула. Применяется к живой
  сессии немедленно: guard читает grants на каждый запрос — следующий
  вызов устройства уже под новым набором; аудит `device.grants`
  (previous → grants).
- **Миграция**: `_migrate_device_grants_v1` (идемпотентна, БЕЗ
  SEED_VERSION bump): активным control-строкам с '' выставляется полный
  набор гранул — поведение уже спаренного телефона не меняется (порядок
  загрузки: сначала v1 scope-миграция active read→control, затем
  гранулы — поэтому активная read-строка к этому шагу уже control);
  мёртвые (revoked/expired) и '[]'-строки не переписываются. Новые
  control-пейринги стартуют с полным набором (store-слой), read-пейринги
  — с '[]'.
- **Viewer**: карточка устройства на `/system/devices` раскрывается в
  переключатели гранул (только для active); переключение = PUT полного
  набора, ответ сервера обновляет строку; read-гранул в списке нет —
  «глобальные read всегда».

Откат: revert-деплой (миграции аддитивные; гранулы остаются в колонке —
старый scope-guard вернёт прежнее поведение без чистки данных).
