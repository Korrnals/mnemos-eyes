...
- Status: **Accepted (owner ratification 2026-09-25)**
  (АРХКОМ 2026-09-24; позиции всех четырёх специалистов — «условно»,
  условия синтезированы председателем и приняты целиком.
  Ратифицировано владельцем 2026-09-25 с поправкой — см. Amendment 1:
  mermaid на всех поверхностях движка)
...
## Amendment 1 (владелец, 2026-09-25): mermaid на всех поверхностях движка

Ратификация с поправкой: **диаграммы mermaid должны корректно обрабатываться
движком на всех поверхностях, включая недоверенные** — отчёты агентов и память
могут нести диаграммы, инертный код-блок на недоверенном профиле больше не
целевое поведение. Комитетская позиция («за горизонтом 1.x», DoS-риск CWE-400)
учитывается защитными условиями реализации, а не отказом от способности:

1. mermaid остаётся ленивым чанком (не попадает в entry), `securityLevel:
   "strict"` — без исключений;
2. **жёсткие размерные лимиты** на недоверенном профиле: cap на размер
   mermaid-фенса и на количество фенсов на поверхность; превышение = честный
   инертный фолбэк (исходный код), не молчаливая деградация;
3. sanitize-гейт один и применяется всегда (инвариант ADR-0017 не ослабевает);
   расширения схемы — только governance-PR;
4. ESLint-инвариант единственности точки mermaid сохраняется: при Ф1 ядро
   получает mermaid как встроенную capability, подключённую в обоих профилях;
5. браузерный смоук (Ф0) ассертит svg-рендер и на недоверенной поверхности.

Критерий приёмки поправки: отчёт агента с корректной диаграммой рендерит её
на странице задачи; hostile-диаграмма (переполнение, запрещённые конструкции)
— честный фолбэк. Сроки — в фазе Ф2 конвергенции (переезд поверхностей),
не раньше устойчивого ядра Ф1.
...
- **ME-011** (Ф0: золотые фикстуры + рендер-смоук + contentClass-гейт)
  ...
  соседней сессии.
- **mermaid на недоверенных поверхностях** — ратифицировано владельцем
  2026-09-25 с защитными условиями (Amendment 1); изначальная комитетская
  позиция («за горизонтом 1.x») отменена решением владельца.
