...
TLS-первое касание честно: если системного доверия к лаб-сертификату
нет, скрипт делает РОВНО ОДИН `curl -k` повтор — только для CA (публичный
материал), проверяет `CA:TRUE`, печатает sha256-fingerprint для сверки с
...
CA руками в `/etc/vesmaro/lab-ca.crt` — `-k`-ветка не выполнится.
